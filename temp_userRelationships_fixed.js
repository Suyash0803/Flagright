...
exports.getUserRelationships = async (req, res) => {
  ...
  try {
    ...
    if (graphResult.records.length > 0) {
      ...
      // Add transaction nodes - CRITICAL FIX: Override the type property
        ...
        if (tx && tx.properties) {
          ...
          nodes.push({
            data: {
              id: txProps.id,
              label: `TX\n$${txProps.amount}`,
              // Spread properties first, then override type
              ...txProps,
              amount: txProps.amount?.toNumber ? txProps.amount.toNumber() : txProps.amount,
              originalType: txProps.type,
              type: 'transaction'  // FORCE this to be 'transaction' for blue color AFTER spread
            }
          });
